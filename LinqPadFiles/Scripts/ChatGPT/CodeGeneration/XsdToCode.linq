...
void Main()
{
    ...
    var outputDirectory = new DirectoryInfo("C:/temp/UBL-Gen");
    var files = Constants.GetUblXsdsFromRootDirectory();
    //    var schemas = SchemaHelper.GetXmlSchemas(files);
    //    schemas.Compile(Handler(), true);
    //
    //    var importer = SchemaHelper.GetXmlSchemaImorter(schemas);
    //    var schemaSet = SchemaHelper.GetXmlSchemaSet(schemas);
    //    schemaSet.Compile();
    //    List<XmlTypeMapping> typeMappings = new List<XmlTypeMapping>();
    //
    //
    //    var cbcSchema = schemaSet.Schemas(Constants.CommonBasicComponentsTargetNamespace).Cast<XmlSchema>().Single();
    //    
    //    
    //    SchemaHelper.SimplifyCommonBasicComponentsTypes(cbcSchema);
    ...
    var preloadFile = files.Where(x => x.Name.StartsWith("UBL-xmldsig-core-schema-")).FirstOrDefault();
    if (preloadFile != null)
    {
        schemaSet.AddSchemaFile(preloadFile.FullName, readerSettings, Handler());
    }

    foreach (var xsdFile in files.Where(x => x.DirectoryName == "maindoc"))
    {
        var schema = schemaSet.AddSchemaFile(xsdFile.FullName, readerSettings, Handler());
    }

    SchemaHelper.ResolveTypeNameClashesByRenaming(schemaSet);
    schemaSet.Compile();

    var abstractMaindocBaseSchema = SchemaHelper.ModifyMaindocSchemasForInheritance(schemaSet.MaindocSchemas());
    ...
    UblCodeGenerator gen = new UblCodeGenerator();
    ...
    var result = gen.GenerateAndSaveCodeFilesBySchema(allCodeDecls, new UblNamespaceManager(schemaSet, "Something"), outputDirectory);
...
public readonly record struct FileData(FileInfo File, string Data);

public class UblCodeGenerator
{
    ...
    public IEnumerable<FileData> GenerateAndSaveCodeFilesBySchema(IEnumerable<CodeTypeDeclaration> allCodeDecls, UblNamespaceManager nsMan, DirectoryInfo outputDir)
    {
        ...
        foreach (var schema in nsMan.Schemas)
        {
            ...
            string csCodeFilename = schema.GetCSharpFilename(outputDir.FullName);
            var file = SaveToFile(codeNamespace, csCodeFilename);
            files.Add(file);
        }
    ...
    public FileData SaveToFile(CodeNamespace ns, string filename)
    {
        ...
        return new FileData(new(filename), sb.ToString());
    }
}
...
public static class XmlSchemaExtensions
{
    ...
    public static XmlSchema AddSchemaFile(this XmlSchemaSet @this, string filename, XmlReaderSettings settings, ValidationEventHandler validationEventHandler)
    {
...
public class UblNamespaceManager
{
...
------------------------------------------------------------------------------";
    ...
    /// <summary>
    /// </summary>
    public UblNamespaceManager(XmlSchemaSet schemaSet, string csDefaultNamespace)
    {
...
public static class SchemaHelper
{
    ...
    public static XmlSchema ReadXmlSchema(FileInfo xsd)
    {
        var schema = XmlSchema.Read(xsd.OpenRead(), null);
        return schema;
    }

    public static XmlSchemaImporter GetXmlSchemaImorter(XmlSchemas schemas)
    {
        XmlSchemaImporter schemaImporter = new XmlSchemaImporter(schemas);
        return schemaImporter;
    }

    public static XmlSchemaSet GetXmlSchemaSet(XmlSchemas schemas)
    {
        XmlSchemaSet schemaSet = new XmlSchemaSet(new NameTable());
        ...
        schemas.ToList().ForEach(x => schemaSet.Add(x));
        schemaSet.Compile();
        return schemaSet;
    }

    public static XmlSchemas GetXmlSchemas(IEnumerable<FileInfo> xsds)
    {
        var schemas = new XmlSchemas();
        foreach (var xsd in xsds)
        {
            var schema = ReadXmlSchema(xsd);
            schemas.Add(schema);
        }
        return schemas;
    }
...
{
    public static void RunSyntaxing(string csharpCode)
    {
        // Generate a syntax tree for the generated C# code
        SyntaxTree syntaxTree = SyntaxFactory.ParseSyntaxTree(csharpCode);

        // Create a compilation based on the syntax tree
        ...
        // Generate the compiled code as an in-memory assembly
        MemoryStream stream = new MemoryStream();
        EmitResult result = compilation.Emit(stream);

        if (result.Success)
        {
            // Load the in-memory assembly into the current app domain
            Assembly assembly = Assembly.Load(stream.GetBuffer());

            // Use reflection to instantiate types and invoke methods from the generated assembly
            // ...
        }
    }
}
...
{
    public static DirectoryInfo GetUblRootDirectory() => new DirectoryInfo(@"C:\repos\ubllarsen\UblLarsen\UBL-2.1\xsd");
    public static IEnumerable<FileInfo> GetUblXsdsFromRootDirectory() => GetUblRootDirectory().EnumerateFiles("*.xsd", SearchOption.AllDirectories);
...
public static class XmlTypeMappingExtensions
{
    public static string GetCSharpCode(this XmlTypeMapping typeMapping)
    {
        StringBuilder sb = new StringBuilder();
        ...
        sb.AppendLine($"namespace {typeMapping.Namespace}");
        sb.AppendLine("{");

        // Generate the class declaration
        sb.AppendLine($"    public class {typeMapping.TypeName}");
        sb.AppendLine("    {");

        // Generate the class members
        //foreach (XmlMapping memberMapping in typeMapping.DerivedTypes[0].RelatedMaps)
        //{
        //    if (memberMapping is XmlTypeMapping)
        //    {
        //        // Nested class
        //        XmlTypeMapping nestedTypeMapping = (XmlTypeMapping)memberMapping;
        //        sb.AppendLine($"        public {nestedTypeMapping.TypeName} {nestedTypeMapping.ElementName} {{ get; set; }}");
        //    }
        //    else
        //    {
        //        // Property
        //        XmlMemberMapping propertyMapping = (XmlMemberMapping)memberMapping;
        //        sb.AppendLine($"        public {propertyMapping.TypeDesc.Name} {propertyMapping.ElementName} {{ get; set; }}");
        //    }
        //}
