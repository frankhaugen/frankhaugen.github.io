...
class NaiveBayesClassifier
{
    ...
    public NaiveBayesClassifier(string[][] trainingData, string[] labels)
    {
        ...
        for (int i = 0; i < trainingData.Length; i++)
        {
            ...
            {
                int featureIndex = Array.IndexOf(logLikelihoods[classIndex], trainingData[i][j]);
                if (featureIndex >= 0)
                {
                    logLikelihoods[classIndex][featureIndex]++;
                }
            }
        }
