...
async Task Main()
{
    ...
    await Parallel.ForEachAsync(generator.XSDs.Keys, async (xsdName, cancellationToken) =>
    {
        var syntaxTree = generator.Generate(xsdName);
        var text = await syntaxTree.GetTextAsync();
        allTypeNames.Add(xsdName);
        var typename = syntaxTree.GetRoot().DescendantNodes().OfType<TypeDeclarationSyntax>().FirstOrDefault()?.Identifier.ValueText;
        if (typename == null)
            return;
...
public static class XmlSchemaToCodeConverter
{
    ...
    public static CodeTypeReference XmlSchemaElementToCodeTypeReference(XmlSchemaElement element, IDictionary<XmlQualifiedName, XmlSchemaComplexType> baseTypes)
    {
        ...
        if (baseTypes.TryGetValue(typename, out var thing) && thing != null)
        {
            var tempType = thing.Datatype?.ValueType;
            if (tempType != null) return new CodeTypeReference(tempType);

            return new CodeTypeReference(typename.Name);
        }

        return new CodeTypeReference(typeof(object));
    }

    public static CodeTypeReference XmlSchemaComplexTypeToCodeTypeReference(XmlSchemaComplexType complexType, IDictionary<XmlQualifiedName, XmlSchemaComplexType> baseTypes)
    {
        var typeName = complexType.QualifiedName.Name;

        if (complexType.BaseXmlSchemaType != null)
        {
            var baseTypeName = complexType.BaseXmlSchemaType.QualifiedName;
            if (baseTypes.TryGetValue(baseTypeName, out var baseType))
            {
                var baseTypeReference = XmlSchemaComplexTypeToCodeTypeReference(baseType, baseTypes);
